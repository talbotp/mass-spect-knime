...
/**
 * Logic behind the Lanczos Algorithm for finding the m most useful extreme
...
public class LanczosSolver {
	 ...
	 */
	public LanczosSolver(RealMatrix A, int eigenIndex, int maxIterations, boolean compareEigenvectors,
			double accuracy) {
		setA(A);
	 ...
	 *            be for convergence to be considered true.
	 */
	public LanczosSolver(RealMatrix A, boolean compareEigenvectors, double accuracy) {
		setA(A);
		setEigenIndex(eigenIndex);
	...
	public EigenValueVectorPair solve() {
		...
		UnitSphereRandomVectorGenerator randVecGen = new UnitSphereRandomVectorGenerator(sizeA);
		...
		for (int i = 1; i < this.maxIterations; i++) {
			...
			if (betas.get(i - 1) != 0) {
				...
				System.out.println("beta_" + i + " = 0, we calculate a new vector.");
				throw new IllegalStateException("Please rerun the algorithm, rng has failed.");
			}
			...
			w_i = (w_dash_i.subtract(v_i.mapMultiply(alphas.get(i)))).subtract(tmp_v.mapMultiply(betas.get(i - 1)));

			// Make matrix V containing v_i as columns.
			RealMatrix V_matrix = makeV(this.A.getRowDimension(), V);

			// Eigendecompose matrix T.
			EigenDecomposition eigdec = new EigenDecomposition(toPrimitive(alphas), toPrimitive(betas));
			...
			int required_index = (this.eigenIndex > i) ? eigvalpairs.get(i - 1).index
			...
			eigvec = V_matrix.operate(eigdec.getEigenvector(required_index)).toArray();
			...
			if (i != 1) {
				...
				if (this.compareEigenvectors) {
					...
					for (int k = 0; k < eigvec.length; k++) {
						if (Math.abs(Math.abs(previousEigvec[k]) - Math.abs(eigvec[k])) > this.accuracy) {
							iterateAgain = true;
							break;
						}
					}
	...
	public static double[] getOrthogonalUnitVectorOfV(ArrayList<double[]> V) {
		...
		if (V.isEmpty())
			throw new IllegalArgumentException("V is empt, we cannot create a vector for this.");
	...
	public void setA(RealMatrix A) {
		if (A == null)
			throw new IllegalArgumentException("The matrix cannot be set as null.");
		if (!A.isSquare())
			throw new IllegalArgumentException("The matrix must be square");
		if (A.getColumnDimension() < 2)
			throw new IllegalArgumentException("The matrix must have a size greater than 1.");
		this.A = A;
	}
	...
	public void setMaxIterations(int maxIterations) {
		if (A == null)
			throw new IllegalStateException("A is equal to null, we must set A before setting anything else.");
		if (maxIterations <= 1)
			throw new IllegalArgumentException("Please use a value of maxIterations which is greater than 1.");
	...
	public void setEigenIndex(int eigenIndex) {
		if (eigenIndex < 0 || eigenIndex >= A.getColumnDimension())
			throw new IllegalStateException("That index is outside the range of the matrix.");
	...
	public void setAccuracy(double accuracy) {
		if (accuracy < 0)
			throw new IllegalArgumentException("Please use a non negative accuracy value.");
		this.accuracy = accuracy;
	}

	/**
	 * Method we use to compare Lanczos with the eigendecomposition as seen in
	 * the Apache commons methods.
	 * 
	 * THIS METHOD WAS FOR WHITE BOX COMPARISONS IN THE DEVELOPMENT PHASE. THIS
	 * METHOD IS NOT TO BE USED EVER.
	 */
	public static void compareLanczosEigen(RealMatrix X, boolean useVectors, double lanczosAccuracy) {

		// If not symmetric then we fail, cannot perform Lanczos.
		if (!NormalizedCuts.isSymmetric(X, 0))
			throw new IllegalArgumentException("Unsymmetric matrix.");

		// Init variables
		LanczosSolver ls;
		int index_eigval2;
		EigenDecomposition eigDec;
		EigenValueVectorPair evp;

		// Actual block
		{
			eigDec = new EigenDecomposition(X);
			System.out.println(Arrays.toString(eigDec.getRealEigenvalues()));
			...
			index_eigval2 = eigPairs.get(1).index;
			System.out.println("Actual value is " + eigDec.getD().getEntry(index_eigval2, index_eigval2));
		}

		// Lanczos (Estimate) block
		{
			int eigIndex = 1;
			int m = X.getColumnDimension();

			ls = new LanczosSolver(X, eigIndex, m, useVectors, lanczosAccuracy);
			evp = ls.solve();
			System.out.println("Estimate values is " + evp.eigenvalue);
		}

		System.out.println("Actual vec is " + Arrays.toString(eigDec.getEigenvector(index_eigval2).toArray()));
		System.out.println("Estimate vec is " + Arrays.toString(evp.eigenvector));

	}

	/**
	 * Make a random RealMatrix which is symmetric and is an OpenMapRealMatrix.
	 * 
	 * This is useful for testing purposes when marking matrices.
	 * 
	 * @param size
	 *            is the width and height of the matrix to return.
	 * @param factor
	 *            is the factor that we multiply by a random element when
	 *            setting values in the matrix.
	 * @return a symmetric RealMatrix.
	 */
	public static RealMatrix makeRandomSymmetricMatrix(int size, int factor) {
		RealMatrix A = new OpenMapRealMatrix(size, size);
		for (int i = 0; i < size; i++) {
			for (int j = i; j < size; j++) {
				double tmp = Math.random() * factor;
				A.setEntry(i, j, tmp);
				A.setEntry(j, i, tmp);
			}
		}
	...
	public static void main(String[] args) {
		int size = 150;
		int factor = 500;
		RealMatrix X = makeRandomSymmetricMatrix(size, factor);
		
		compareLanczosEigen(X, true, 0.000000001);
	}
