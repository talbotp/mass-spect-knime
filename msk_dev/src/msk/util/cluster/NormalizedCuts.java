...
import msk.util.cluster.LanczosSolver.EigenValueVectorPair;
		...
		for (BiPartition leaf : partitions) {
			...
			if (leaf.partition == null) { // First iteration of method.
				...
				if ((getNCutType() != PARTITION_BY_NCUT && getCurrentNClusters() < getNClusters())
					...
					// Stop because the maximum number of clusters has been reached.
					break;
				}
			}
			...
			//NOT HERE BECAUSE IT CAN BREAK KNIME.
			//displayClusters();
